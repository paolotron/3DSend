...
class FaissNN(object):
    def __init__(self, on_gpu: bool = False, num_workers: int = 4, metric='l2') -> None:
        ...
        assert metric in ('l2', 'inner')
        self.metric = metric

    def cpu(self):
        self.search_index = self._index_to_cpu(self.search_index)
        self.on_gpu = False
...
class NetworkFeatureAggregator(torch.nn.Module):
    ...
    def __init__(self, backbone, layers_to_extract_from, device):
        ...
        for extract_layer in layers_to_extract_from:
            ...
            if "." in extract_layer:
                ...
                blocks = extract_layer.split(".")
                network_layer = backbone.__dict__["_modules"][blocks[0]]
                for extract_idx in blocks[1:]:
    ...
    def feature_dimensions(self, input_shape, index=-1):
        """Computes the feature dimensions for all layers given input_shape."""
        _input = torch.ones([1] + list(input_shape)).to(self.device)
        _output = self(_input)
        if index != -1:
            return [_output[layer][index].shape[1] for layer in self.layers_to_extract_from]
...
class NearestNeighbourScorer(object):
    ...
    def predict(
        ...
        Args:
             query_features: [dict of np.arrays] List of np.arrays
