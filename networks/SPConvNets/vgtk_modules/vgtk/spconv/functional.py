...
def inter_pooling_naive(inter_idx, sample_idx, feats, alpha=0.5):
    ...
    grouped_feats = batched_index_select(add_shadow_feature(feats), 2, inter_idx.long().view(b, -1)).view(b, -1, p, pnn,
                                                                                                          a)
...
def inter_zpconv_grouping(xyz, feats, stride, n_neighbor,
    ...
    if inter_idx is None:
        ...
        grouped_xyz, ball_idx, idx, new_xyz = inter_zpconv_grouping_ball(xyz, stride,
                                                                         radius * radius_expansion, n_neighbor,
                                                                         lazy_sample)
...
# def compute_anchor_kernel_weights(anchors_old, anchor_out, kpts, ann, aperature, sigma=1e-1):
#     '''
#     Anchor weights for intrasphere convolution
#     param:
#         anchors_in: ax3
...
#     # axann
#     anchor_out = anchors_old if anchor_out is None else anchor_out

#     # a_out x ann
#     angles, idx = anchor_knn(anchors_old, anchor_out, k=ann, metric='angular')
#     # a_out x ks x ann
