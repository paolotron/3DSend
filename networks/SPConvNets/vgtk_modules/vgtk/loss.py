...
class MultiTaskDetectionLoss(torch.nn.Module):
    ...
    def forward(self, wts, label, y, gt_R, gt_T=None):
        ...
        if na == 1:
            ...
            #                        anchors_src, select_RAnchor, self.anchors_old[label])

            ##############################################
            # gt_Rs = torch.einsum('baij, bajk, balk -> bail',\
            #                      anchors_src, gt_R, self.anchors_old[label])
            ...
            # option 2: regress nearby anchors_old within an angular threshold
...
class TripletBatchLoss(nn.Module):
    def __init__(self, opt, anchors, sigma=2e-1, \
        ...
        '''
            anchors_old: na x 3 x 3, default anchor rotations
        ...
        # anchors_old = sgtk.functinoal.get_anchors()
