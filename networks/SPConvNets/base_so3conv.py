...
from vgtk import spconv as zptk
from vgtk import so3conv as sptk
...
def preprocess_input(x, na, add_center=True):
    ...
    xyz = x[:, :, :3]
    return zptk.SphericalPointCloud(xyz.permute(0, 2, 1).contiguous(), sptk.get_occupancy_features(x, na, add_center),
                                    None)
...
class ClsOutBlockPointnet(nn.Module):
    ...
    def forward(self, x, label=None, penultimate=False):
        x_out = x.feats
